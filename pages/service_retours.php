...
<body>
  ...
  <div class="col-10" style="margin-left: 18%;">
    ...
    <div class="container my-4">
      ...
      <h3 class="p-4">CONFIRMED INVOICES</h3>

      <div class="col-md-6">
        <div class="h-100 p-5 text-white rounded-3" style="background:#002089;">
          <h2>YOUR CONFIRMED RESERVATIONS TOTAL</h2>
          <small>The total sum of all confirmed invoices and received money with reserved rooms</small>
          <?php
          $sqlsum = "SELECT SUM(total) as count FROM reservation WHERE confirmation =1";
          $result = mysqli_query($conn, $sqlsum);
          $row = mysqli_fetch_assoc($result);

          $count = $row['count'];

          echo "<br><h2>" . $count . " MAD"."<h2/>";
          ?>
        </div>

      </div>

      <h3 class="p-4">ALL INVOICES</h3>

      <div class="col-md-6">
        <div class="h-100 p-5 text-white rounded-3" style="background:red;">
          <h2>YOUR TOTAL</h2>
          <small>The total sum of all confirmed and unhold invoices</small>
          <?php
          $sqlsum = "SELECT SUM(total) as count FROM reservation WHERE 1";
          $result = mysqli_query($conn, $sqlsum);
          $row = mysqli_fetch_assoc($result);
